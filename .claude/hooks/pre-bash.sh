...
# ── 위험 패턴 검사 (pattern|reason 쌍 배열, bash 3 호환) ──────────
PATTERNS=(
  "rm -rf /"             "루트 삭제 시도"
  "rm -rf ~"             "홈 디렉토리 삭제 시도"
  "rm -rf \*"            "와일드카드 재귀 삭제"
  "rm -rf \."            "현재 디렉토리 삭제"
  "git reset --hard"     "하드 리셋 (작업 유실 위험)"
  "git push.*--force"    "강제 푸시"
  "git push.*-f "        "강제 푸시"
  "git push origin main$"    "main 직접 푸시 (PR 없이)"
  "git push origin master$"  "master 직접 푸시"
  "DROP TABLE"           "DB 테이블 삭제"
  "DROP DATABASE"        "DB 삭제"
  "mkfs"                 "파일시스템 포맷"
  "dd if="               "디스크 직접 쓰기"
  ":\(\){ :|:&"          "Fork 폭탄"
  "chmod -R 777 /"       "루트 권한 개방"
  "Remove-Item.*-Recurse.*-Force.*C:" "Windows 드라이브 삭제"
  "Format-Volume"        "볼륨 포맷"
)
